...
PAPER = ROOT / "paper"
DOWNLOADS_PDF = Path.home() / "Downloads" / "107.pdf"
DESKTOP_PDF = Path.home() / "Desktop" / "107.pdf"
FACTORY_ROOT_PDF = ROOT.parent / "107.pdf"
CHILD_ROOT_PDF = ROOT / "107.pdf"

EXPECTED_ROWS = {
    "dataset_summary_rows": 576,
    "main_cell_rows": 92160,
    "main_group_rows": 9216,
    "seed_metric_rows": 1280,
    "metric_rows": 128,
    "hard_seed_rows": 160,
    "hard_metric_rows": 16,
    "hard_pairwise_rows": 15,
    "ablation_cell_rows": 7200,
    "ablation_seed_rows": 100,
    "ablation_metric_rows": 10,
    "stress_cell_rows": 43200,
    "stress_seed_rows": 600,
    "stress_metric_rows": 60,
    "fixed_risk_cell_rows": 46080,
...
def main():
    ...
    for gate in ["success_gate", "diagnostic_gate", "safety_gate", "pairwise_gate", "ablation_gate", "stress_gate", "fixed_risk_gate"]:
        if not summary["gates"].get(gate):
            raise AssertionError(f"local gate failed: {gate}")
    if summary["gates"].get("scope_gate"):
        raise AssertionError("scope gate should fail without external robotics evidence")
    ...
    for key, expected in EXPECTED_ROWS.items():
        ...
        path = ROW_FILES[key]
        counted = count_csv(path)
        if counted != expected:
            raise AssertionError(f"{path} row count expected {expected}, got {counted}")
    ...
    if not DOWNLOADS_PDF.exists():
        raise AssertionError("Downloads/107.pdf missing")
    if DESKTOP_PDF.exists():
        raise AssertionError("visible Desktop 107.pdf must not exist")
    if FACTORY_ROOT_PDF.exists():
        raise AssertionError("factory-root 107.pdf must not exist")
    if CHILD_ROOT_PDF.exists():
        raise AssertionError("child-root 107.pdf must not exist")
    ...
    print(f"validated Paper 107 artifacts: pages={pages}, sha256={paper_hash}")
